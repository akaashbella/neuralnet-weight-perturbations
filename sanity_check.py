...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        print("Training MLP clean (1 epoch)...")
        train_one("mlp", train_loader, noisy=False, seed=0, device=device, save_path=clean_path, epochs=1)
        print("Training MLP noisy (1 epoch)...")
        train_one("mlp", train_loader, noisy=True, seed=0, device=device, save_path=noisy_path, epochs=1)

        print("\nRobustness at α_test ∈ {0, 0.1}:")
        _, sweep_clean = load_and_sweep("mlp", clean_path, test_loader, device, alpha_list=alpha_sanity)
        _, sweep_noisy = load_and_sweep("mlp", noisy_path, test_loader, device, alpha_list=alpha_sanity)
